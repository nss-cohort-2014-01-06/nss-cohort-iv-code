...
module('Database Operations', {
  setup: function() {
    collection = 'c' + Math.floor(Math.random() * 100000);
    database = 'alphax';
  },
  teardown: function() {
  }
...
asyncTest('Client#save', function() {
  var c1 = new Client('Bob Smith', 100000);
  var p1 = new Portfolio('Tech Stocks');
  var s1 = new Stock('AAPL', 50, 250);
  ...
  c1.save(function(client){
    ok(client._id, 'got id');
    start();
  });
});

asyncTest('Client#find', function() {
  var c1 = new Client('Bob', 100000);
  var c2 = new Client('Sam', 200000);
  var c3 = new Client('Liz', 300000);

  c1.save(function(){
    c2.save(function(){
      c3.save(function(){
        Client.find('Sam', function(client){
          deepEqual(client.name, 'Sam', 'found sam');
          start();
        });
      });
    });
